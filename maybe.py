...
class EnhancedPositionalEncoding(nn.Module):
    def _init_(self, d_model, max_len=1000, dropout=0.2):
        super()._init_()
...
class FocalLoss(nn.Module):
    def _init_(self, alpha=None, gamma=4.0, label_smoothing=0.1):
        super()._init_()
        self.alpha = alpha
        self.gamma = gamma
...
# Save class weights
np.savez(os.path.join(output_dir, "class_weights.npz"), class_weights=class_weights)
